...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$desktop=[Environment]::GetFolderPath('DesktopDirectory'); $shortcut=Join-Path $desktop 'EcoSensor Servidor.lnk'; $target=Join-Path '%~dp0' 'run.bat'; $workdir='%~dp0'; $icon=Join-Path $workdir 'app\EcoSensor_WiFi.ico'; $shell=New-Object -ComObject WScript.Shell; $link=$shell.CreateShortcut($shortcut); $link.TargetPath=$target; $link.WorkingDirectory=$workdir; $link.Description='EcoSensor Servidor Portable'; if (Test-Path $icon) { $link.IconLocation=$icon }; $link.Save()" >nul 2>&1
if %errorlevel%==0 (
...
echo Ahora puedes ejecutar el servidor con run.bat o desde el acceso directo del escritorio.
echo.
pause
