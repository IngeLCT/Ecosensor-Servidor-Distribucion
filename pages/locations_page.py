...
def _location_cluster_csv_text(cluster: LocationCluster) -> str:
    ...
    for row in cluster.rows:
        writer.writerow({
            ...
            'Ubicacion': cluster.display_location,
            'Latitud': f"{float(row.get('_lat')):.6f}",
            'Longitud': f"{float(row.get('_lon')):.6f}",
            'PM1.0': format_value(row.get('pm1p0')),
            'PM2.5': format_value(row.get('pm2p5')),
            'PM4.0': format_value(row.get('pm4p0')),
            'PM10.0': format_value(row.get('pm10p0')),
            'VOC': format_value(row.get('voc')),
            'NOx': format_value(row.get('nox')),
            'CO2': format_value(row.get('co2'), 0),
            'Temperatura': format_value(row.get('temp')),
